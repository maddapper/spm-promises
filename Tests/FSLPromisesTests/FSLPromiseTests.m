...
@interface FSLPromiseTests : XCTestCase
@end

@implementation FSLPromiseTests
...
- (void)testPromiseConstructorPending {
  // Arrange & Act.
  FSLPromise<NSNumber *> *promise = [FSLPromise pendingPromise];
...
- (void)testPromiseConstructorResolvedWithValue {
  // Arrange & Act.
  FSLPromise<NSNumber *> *promise = [FSLPromise resolvedWith:@42];
...
- (void)testPromiseConstructorResolvedWithError {
  ...
  FSLPromise<NSNumber *> *promise = [FSLPromise resolvedWith:error];
...
- (void)testPromiseFulfillWithError {
  ...
  // Act.
  [promise fulfill:(id)[NSError errorWithDomain:FSLPromiseErrorDomain code:42 userInfo:nil]];
...
- (void)testPromiseNoDoubleReject {
  ...
  // Act.
  [promise reject:[NSError errorWithDomain:FSLPromiseErrorDomain code:42 userInfo:nil]];
  [promise reject:[NSError errorWithDomain:FSLPromiseErrorDomain code:13 userInfo:nil]];
...
- (void)testPromisePendingDealloc {
  FSLPromise __weak *weakPromise;

  @autoreleasepool {
    XCTAssertNil(weakPromise);
    weakPromise = [FSLPromise pendingPromise];
