...
@implementation FSLPromise (RaceAdditions)
...
@implementation FSLPromise (DotSyntax_RaceAdditions)

+ (FSLPromise * (^)(NSArray *))race {
  return ^(NSArray *promises) {
    return [self race:promises];
  };
}

+ (FSLPromise * (^)(dispatch_queue_t, NSArray *))raceOn {
