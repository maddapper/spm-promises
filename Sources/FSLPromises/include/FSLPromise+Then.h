...
@interface FSLPromise<Value>(ThenAdditions)

typedef id __nullable (^FSLPromiseThenWorkBlock)(Value __nullable value) NS_SWIFT_UNAVAILABLE("");
 ...
 */
- (FSLPromise *)then:(FSLPromiseThenWorkBlock)work NS_SWIFT_UNAVAILABLE("");
...
- (FSLPromise *)onQueue:(dispatch_queue_t)queue
                   then:(FSLPromiseThenWorkBlock)work NS_REFINED_FOR_SWIFT;

@end

/**
 Convenience dot-syntax wrappers for `FSLPromise` `then` operators.
 Usage: promise.then(^id(id value) { ... })
 */
@interface FSLPromise<Value>(DotSyntax_ThenAdditions)

- (FSLPromise* (^)(FSLPromiseThenWorkBlock))then FSL_PROMISES_DOT_SYNTAX NS_SWIFT_UNAVAILABLE("");
- (FSLPromise* (^)(dispatch_queue_t, FSLPromiseThenWorkBlock))thenOn FSL_PROMISES_DOT_SYNTAX
