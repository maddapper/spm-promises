 ...
 */
@interface FSLPromise<__covariant Value> : NSObject

/**
 Default dispatch queue used for `FSLPromise`, which is `main` if a queue is not specified.
 */
...
#ifdef FSL_PROMISES_DOT_SYNTAX_IS_DEPRECATED
#define FSL_PROMISES_DOT_SYNTAX __attribute__((deprecated))
#else
#define FSL_PROMISES_DOT_SYNTAX
#endif

@interface FSLPromise<Value>(DotSyntaxAdditions)

/**
 Convenience dot-syntax wrappers for FSLPromise.
 Usage: FSLPromise.pending()
        FSLPromise.resolved(value)

 */
+ (instancetype (^)(void))pending FSL_PROMISES_DOT_SYNTAX NS_SWIFT_UNAVAILABLE("");
+ (instancetype (^)(id __nullable))resolved FSL_PROMISES_DOT_SYNTAX NS_SWIFT_UNAVAILABLE("");
