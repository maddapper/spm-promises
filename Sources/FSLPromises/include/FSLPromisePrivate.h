...
/**
 Miscellaneous low-level private interfaces available to extend standard FSLPromise functionality.
 */
@interface FSLPromise<Value>()

typedef void (^FSLPromiseOnFulfillBlock)(Value __nullable value) NS_SWIFT_UNAVAILABLE("");
typedef void (^FSLPromiseOnRejectBlock)(NSError *error) NS_SWIFT_UNAVAILABLE("");
typedef id __nullable (^__nullable FSLPromiseChainedFulfillBlock)(Value __nullable value)
    NS_SWIFT_UNAVAILABLE("");
typedef id __nullable (^__nullable FSLPromiseChainedRejectBlock)(NSError *error)
    NS_SWIFT_UNAVAILABLE("");
...
- (void)observeOnQueue:(dispatch_queue_t)queue
               fulfill:(FSLPromiseOnFulfillBlock)onFulfill
                reject:(FSLPromiseOnRejectBlock)onReject NS_SWIFT_UNAVAILABLE("");
...
- (FSLPromise *)chainOnQueue:(dispatch_queue_t)queue
              chainedFulfill:(FSLPromiseChainedFulfillBlock)chainedFulfill
               chainedReject:(FSLPromiseChainedRejectBlock)chainedReject NS_SWIFT_UNAVAILABLE("");
