...
@interface FSLPromise<Value>(ReduceAdditions)

typedef id __nullable (^FSLPromiseReducerBlock)(Value __nullable partial, id next)
 ...
 */
- (FSLPromise *)reduce:(NSArray *)items
               combine:(FSLPromiseReducerBlock)reducer NS_SWIFT_UNAVAILABLE("");
...
/**
 Convenience dot-syntax wrappers for `FSLPromise` `reduce` operators.
 Usage: promise.reduce(values, ^id(id partial, id next) { ... })
 */
@interface FSLPromise<Value>(DotSyntax_ReduceAdditions)

- (FSLPromise * (^)(NSArray *, FSLPromiseReducerBlock))reduce FSL_PROMISES_DOT_SYNTAX
    NS_SWIFT_UNAVAILABLE("");
